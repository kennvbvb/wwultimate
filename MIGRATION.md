...
> **สถานะ: ย้ายเสร็จแล้วครบทุกเฟส (เฟส 0–6)**
> เอกสารนี้เก็บไว้เป็นบันทึกว่าทำไมถึงเลือกทางนี้ ไม่ใช่รายการงานที่ค้างอยู่แล้ว
> สภาพปัจจุบันของระบบให้ดูที่ `CLAUDE.md` ส่วนวิธีติดตั้งและ deploy อยู่ใน `README.md`
>
> สิ่งที่ต่างจากแผนเดิมเล็กน้อย:
> - เทสต์ชั้นจัดเก็บข้อมูลรันบน Postgres จริง ไม่ใช้ `pg-mem` (ของจำลองไม่รู้จัก `SELECT ... FOR UPDATE`)
> - สตรีมใช้การ poll เลขเวอร์ชันฝั่งเซิร์ฟเวอร์แทน `LISTEN`/`NOTIFY` ตามที่เอกสารเปิดทางไว้
> - `VILLAGE_IMPACT_VERIFIED` เปิดด้วย environment variable แทนการแก้ไฟล์ตรรกะ
> - เพิ่มเทสต์กันจอสาธารณะรั่วบทบาท 3 ข้อ (รวมเป็น 80 ข้อ)
> - งานพัฒนาอยู่บน branch `claude/new-session-h50h7z` ไม่ใช่ `vercel-migration`
