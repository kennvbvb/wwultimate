...
| ชุดทดสอบ | **ผ่าน 150 / ล้มเหลว 0** (`npm test`) |
...
lib/
  ...
  api.ts                        34  แปลง Error เป็น HTTP status (ตัวแปลงจริงอยู่ที่ errors.ts)
  errors.ts                         GameError/AuthError + classifyError (ฐานข้อมูลล่ม/schema ตามหลัง → 503)
  schema.ts                         ตรวจว่าตารางและคอลัมน์ที่โค้ดต้องใช้มีครบตาม migration หรือยัง (อะแดปเตอร์ของ errors.ts)
...
- **`/api/health`** ตรวจฐานข้อมูลจริง **และตรวจว่ารัน migration ครบหรือยัง** (`lib/schema.ts`)
  ใช้กับ uptime monitor ได้ ตอบ 503 เมื่อต่อฐานข้อมูลไม่ได้หรือ schema ตามหลัง และต้องไม่บอกรายละเอียดภายใน
  **เพิ่ม migration ใหม่เมื่อไร ต้องเพิ่มแถวใน `REQUIRED` ของ `lib/schema.ts` ด้วย** ไม่งั้นการตรวจจะมองไม่เห็น
...
| `tests/apierrors.test.js` | 7 | ฐานข้อมูลล่ม/ยังไม่ได้รัน migration ต้องตอบ 503 แบบไม่บอกโฮสต์ พอร์ต หรือชื่อคอลัมน์ |
| `tests/schema.test.js` | 4 | ตรวจเจอว่าฐานข้อมูลตามหลัง migration และบอกได้ว่าต้องรันไฟล์ไหน |
...
npm run smoke      # ยิง API ครบทุกคำสั่ง + SSE + แอดมิน + สถิติ + health/headers/retention (51 ข้อ)
